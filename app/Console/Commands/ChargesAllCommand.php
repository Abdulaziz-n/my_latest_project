...
class ChargesAllCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'charge:monthly';
...
//        $this->info("TARIFFF UN COUNT: {$c}");
        ...
        for ($i = 0; $i <= 30; $i++) {
            foreach ($tarrifs as $tarrif) {
                $this->info("START {$tarrif} DAY: ". Carbon::today()->subDays($i)->format("Y-m-d"));
                if ($tarrif >= 2604) {
                    $count = ChargePremium::query()->where('status', true)
                        ->whereBetween('date', [Carbon::today()->subDays($i)->format("Y-m-d"), Carbon::today()->subDays($i)->endOfDay()->format("Y-m-d")])
